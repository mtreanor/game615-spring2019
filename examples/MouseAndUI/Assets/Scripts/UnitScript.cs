				...
				if (EventSystem.current.IsPointerOverGameObject() == false) {
					...
					if (Physics.Raycast(ray, out hit, layerMask)) {
						if (hit.collider.gameObject.layer == LayerMask.NameToLayer("Ground")) {
							//We will only go in here if the raycast hit something with the layer "Ground"
							//Set our target destination to where the Raycast hit.
							targetDestination = hit.point;

							//Make sure the dialogue box is off if we start moving
							//Another choice woud have been to have the doalogue box follow the moving player
							gameManager.dialogeObject.SetActive(false);
						}
					} else {
